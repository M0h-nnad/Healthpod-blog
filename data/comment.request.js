...
const createComment = async (req, res, next) => {
  ...
  newComment.save(async (err) => {
    ...
    res.status(200).json({ massage: "Comment Created", newComment });
    next()
  });
};
...
const updateComment = async (req, res, next) => {
  ...
  updatedComment.save((err) => {
    ...
    res.status(200).json({ massage: "Comment Updated", newComment });
    next()
  });
};
...
const DeleteComment = async (req, res, next) => {
  Comment.deleteOne({ _id: req.params.id }).then((result) => {
    res
      .status(200)
      .json({ massage: "The Comment Deleted !", deletedId: req.params.id });
    next();
  });
};
...
const getPostComments = async (req, res, next) => {
  ...
  if (!post) {
    return res.status(404).json({ massage: "Post is Not Found" });
  }
